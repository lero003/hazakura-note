...
| --- | --- | --- | --- |
| Image paste | Clipboard image paste can call `save_pasted_image`, write supported image bytes into workspace `assets/`, and insert Markdown. Pasted `assets/...`, `./assets/...`, and `/assets/...` images render in preview/export/print through the existing workspace-image validation path and are inlined as data: URIs. | Alt text, visible error/status, duplicate pasted-image cleanup, and smoke coverage are incomplete. | Add smoke coverage for the image paste + preview + export flow. |
